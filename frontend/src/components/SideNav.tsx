...
export function SideNav() {
  ...
  const navLinks: NavLink[] = [
    ...
    {
      href: "/orderbook/admin",
      label: "Order Book Admin",
      requiresAdmin: true,
      icon: (
        <svg
          className="w-5 h-5"
    ...
    {
      href: "/admin/liquidity/positions",
      label: "My Positions",
      showAlways: true,
      icon: (
        <svg
  ...
  // Filter links based on admin role
  const visibleLinks = navLinks.filter((link) => {
    // Always show links marked as showAlways
    if (link.showAlways) return true;

    // Show admin links only if user has admin role

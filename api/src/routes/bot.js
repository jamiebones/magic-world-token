...
router.post('/trade/execute', checkBotEnabled, async (req, res) => {
    try {
        ...
        const trade = new Trade({
            ...
            status: 'PENDING',
            marketPriceAtExecution: prices.mwtBnb.price.toString(),
            pegDeviation: deviation.usd.deviationPercent,
            liquidity: {
                totalUSD: prices.liquidity.totalLiquidityUSD,
                mwtReserve: prices.liquidity.mwtLiquidity.toString(),
                bnbReserve: prices.liquidity.bnbLiquidity.toString()
            }
        });
...
router.post('/trade/estimate', async (req, res) => {
    try {
        ...
        const priceImpact = tradeExecutor.calculatePriceImpact(
            ...
            parseFloat(currentPrice.mwtBnbPrice)
        );
...
router.put('/config', async (req, res) => {
    try {
        const config = await BotConfig.getDefault();
        const { thresholds, limits, slippage, strategy, safety } = req.body;
